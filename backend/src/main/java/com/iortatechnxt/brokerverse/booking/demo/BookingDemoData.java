package com.iortatechnxt.brokerverse.booking.demo;
...
/**
 * Books the demo accounts of V988 through the real booking service (demo profile only, idempotent):
...
@Profile("demo")
@Order(80)
public class BookingDemoData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(BookingDemoData.class);
  ...
  public void run(ApplicationArguments args) {
    ...
    post(
        new EndorsementPosting(
            ...
            "DEMO:CANCEL:940002"));
    LOG.info("Booking demo data: {} accounts booked, one endorsement and one cancellation", booked);
  }
  ...
  private int book(String arn, LocalDate date) {
    try {
      ...
      LOG.warn("Demo booking of {} skipped: {}", arn, ex.getMessage());
      return 0;
    }
  }
  ...
  private void post(EndorsementPosting posting) {
    try {
      ...
      LOG.warn("Demo endorsement of {} skipped: {}", posting.arn(), ex.getMessage());
    }
  }
