 ...
 * decision D4): claim count, open claims, the status of each claim, paid, outstanding and total, for
 * an account (ARN) and optionally one policy year. Public read API of {@code brokerclaims}: Renewal
 * (With Claim Y/N, number and status of claims, the CLAIMS check, the Account History tab) and the
 * account page's Claims tab read the loss experience only through this service; nothing depends on
 * {@code brokerclaims} otherwise. The figures are those of the Loss Experience report ({@link
 * LossLines}).
 */
@Service
