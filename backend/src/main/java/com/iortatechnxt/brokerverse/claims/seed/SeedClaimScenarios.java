...
/**
 * Runs one planned seed claim through its lifecycle with the claims services: the claims officer
...
final class SeedClaimScenarios {
  ...
  private final Map<SeedClaimPlan.Scenario, Consumer<Steps>> scenarios =
      new EnumMap<>(SeedClaimPlan.Scenario.class);
  ...
  SeedClaimScenarios(
    ...
    scenarios.put(SeedClaimPlan.Scenario.OPEN, s -> s.approvePending(APPROVE_AFTER));
    scenarios.put(SeedClaimPlan.Scenario.PARTIAL, this::partial);
    scenarios.put(SeedClaimPlan.Scenario.FINAL, s -> finalSettlement(s, s.pc.lossReserve()));
    scenarios.put(SeedClaimPlan.Scenario.REDUCED_FINAL, this::reducedFinal);
    scenarios.put(SeedClaimPlan.Scenario.REPUDIATED, this::repudiate);
    scenarios.put(SeedClaimPlan.Scenario.PENDING_INCREASE, this::pendingIncrease);
    scenarios.put(SeedClaimPlan.Scenario.REOPENED, this::reopened);
    scenarios.put(SeedClaimPlan.Scenario.WITHDRAWN, this::withdraw);
    scenarios.put(SeedClaimPlan.Scenario.PENDING_SETTLEMENT, this::pendingSettlement);
    scenarios.put(SeedClaimPlan.Scenario.SALVAGE, this::salvage);
  }
  ...
  Claim run(Long companyId, PlannedClaim pc) {
    boolean withdrawn = pc.scenario() == SeedClaimPlan.Scenario.WITHDRAWN;
  ...
  private static ClaimRequest request(Long companyId, PlannedClaim pc, boolean reserve) {
    ...
    SeedLoss loss = SeedLoss.of(pc.policy().businessLine());
