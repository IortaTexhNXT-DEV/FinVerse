package com.iortatechnxt.brokerverse.collections.seed;
...
/**
 * Seed storyline of the Collections plans and escalations (wave C1-B, seed profile only,
 * idempotent), each step signed in as the SIT/UAT user whose job it is:
 *
 * <ul>
 *   <li>the collection handler ({@code mktcoll}) makes an annual policy-year plan for the
 *       three-year seed account and generates its statement of account for each of the three
 ...
 *   <li>a promise kept on ARN-2026-940004 (paid by the cashiering seed), a promise running on the
...
@Profile("seed")
@Order(110)
public class CollectionsPlansSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(CollectionsPlansSeedData.class);
  ...
  private final SeedMultiYearAccount account;
   ...
   * @param account the three-year seed account
   * @param users seed sign-in
   * @param clock clock
   */
  @SuppressWarnings("java:S107") // constructor injection
  public CollectionsPlansSeedData(
  ...
  public void run(ApplicationArguments args) {
    if (planRecords.count() == 0) {
      bookingInvoice(QUARTERLY_ARN)
          .ifPresentOrElse(
              this::load,
              () -> LOG.warn("Collections plans seed skipped: {} is not booked", QUARTERLY_ARN));
    }
  }
  ...
  private void load(OpsInvoice quarterly) {
    ...
    LOG.info("Collections plans seed data: plans, statements, promises and escalations loaded");
  }
  ...
  private static void step(String name, Runnable work) {
    try {
      ...
      LOG.warn("Collections plans seed step '{}' skipped: {}", name, ex.getMessage());
    }
  }
}
