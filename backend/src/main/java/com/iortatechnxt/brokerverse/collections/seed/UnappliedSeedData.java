...
/**
 * Collector side of the unapplied-payments seed storyline (wave C1-C, seed profile only,
 * idempotent), after the cashier received the seed payments without a match:
 *
 * <ul>
 ...
 * ({@code cashiering.seed.CollectorRequestSeedData}). A step that fails is logged and skipped.
 */
@Component
@Profile("seed")
@Order(126)
public class UnappliedSeedData implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    try {
      ...
      LOG.info("Unapplied collector seed data recorded");
    } catch (RuntimeException ex) {
      LOG.warn("Unapplied collector seed data skipped: {}", ex.getMessage());
    }
  }
