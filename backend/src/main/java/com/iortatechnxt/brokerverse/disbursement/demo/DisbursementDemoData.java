...
/**
 * Demo masters and storyline of Disbursement (demo profile only, idempotent; V999 is full, design
...
@Profile("demo")
@Order(97)
public class DisbursementDemoData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(DisbursementDemoData.class);
  ...
  private void masters(Long companyId) {
    payee(companyId, INSURER, "INSURER", "MAPFRE Insular (Demo)", DisbursementMode.CHECK, null);
    payee(
        companyId,
        CLIENT,
        "CLIENT",
        "Demo Client Refund Payee",
            ...
            () ->
                payees.create(
                    ...
                    new PayeeDetails(
                        ...
                        name,
                        "Makati City",
                        "disbursement-demo@brokerverse-demo.ph",
                        null,
                        mode,
  ...
  private void remittance(Long companyId) {
    List<RemittanceBatch> inReview =
        batches.findByStageInOrderByIdAsc(List.of(BatchStage.REVIEW_IN_PROCESS)).stream()
            ...
            .toList();
    if (inReview.size() < 2) {
      LOG.info("Disbursement demo: the remittance batch in review stays with Operations");
    ...
    payee(
        ...
        null);
    users.run("remit", () -> batchService.submit(batch.getId(), "Checked for Disbursement demo"));
    ...
    LOG.info("Disbursement demo: remittance {} paid by check", approved.getBatchNo());
  }
  ...
  private static void step(String name, Runnable work) {
    try {
      ...
      LOG.warn("Disbursement demo {} skipped: {}", name, ex.getMessage());
    }
  }
}
