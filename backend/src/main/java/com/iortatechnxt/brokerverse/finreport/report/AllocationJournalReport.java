 ...
 *
 * <p>Definition used in BrokerVerse: an <b>allocation JV</b> is a posted ADJUSTMENT or ACCRUAL
 * journal that credits a prepaid (asset) account and debits expense accounts – the periodic
 * amortisation of a prepayment (Dr expense, Cr prepaid). Each expense debit is one allocation line;
 * the allocation period is the calendar month of the JV date (allocated from / to) and the
 * allocated date is the JV date. The optional Prepaid A/c range restricts the credited asset
 * accounts; when a JV credits several prepaid accounts, its expense lines are listed under the
 * first one.
 */
@Component
