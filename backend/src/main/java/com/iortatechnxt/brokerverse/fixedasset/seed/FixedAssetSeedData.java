package com.iortatechnxt.brokerverse.fixedasset.seed;
...
/**
 * Seed profile only: builds a fixed asset register for the seed company through the services, so
...
@Profile("seed")
@Order(70)
public class FixedAssetSeedData implements ApplicationRunner {
  ...
  private static final List<SeedAsset> ASSETS =
      ASSET_TABLE.lines().map(line -> SeedAsset.parse(line.split("\\|"))).toList();
  ...
  private void register(Long companyId, SeedAsset d) {
    FixedAsset asset =
        assets
  ...
  private FixedAssetRequest request(Long companyId, SeedAsset d) {
  ...
  private record SeedAsset(
    ...
    static SeedAsset parse(String[] fields) {
      Iterator<String> f = List.of(fields).iterator();
      return new SeedAsset(
