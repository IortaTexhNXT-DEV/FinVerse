package com.iortatechnxt.brokerverse.frbs.seed;
...
/**
 * Seed storyline of the accounting report pack and service fee (seed profile only, idempotent; V999
...
@Profile("seed")
@Order(98)
public class FrbsSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(FrbsSeedData.class);
  ...
  private static final int SEED_RULE_PRIORITY = 9000;
  ...
  private static void step(String name, Runnable work) {
    try {
      ...
      LOG.warn("FRBS seed step '{}' skipped: {}", name, ex.getMessage());
    }
  }
}
