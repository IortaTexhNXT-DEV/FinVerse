package com.iortatechnxt.brokerverse.investment.seed;
...
/**
 * Seed profile only: builds an investment portfolio for the seed company through the services.
...
@Profile("seed")
@Order(70)
public class InvestmentSeedData implements ApplicationRunner {
  ...
  private static final List<SeedHolding> HOLDINGS = table(IDENTITIES, TERMS);
  ...
  private void acquire(Long companyId, SeedHolding d) {
    InvestmentHolding h =
        holdings
  ...
  private List<SeedEvent> events(Long companyId, SeedHolding d, YearMonth month) {
    ...
    List<SeedEvent> events = new ArrayList<>(couponEvents(h, month));
    LocalDate maturity = h.getMaturityDate();
    if (maturity != null && YearMonth.from(maturity).equals(month)) {
      events.add(new SeedEvent(maturity, () -> mature(h)));
    }
    ...
    if (SOLD.equals(d.code()) && month.equals(YearMonth.from(SALE_DATE))) {
      ...
      events.add(new SeedEvent(SALE_DATE, () -> holdingEvents.sell(h.getId(), sale)));
    }
    return events;
  }

  private boolean needsValuation(InvestmentHolding h, SeedHolding d, YearMonth month) {
  ...
  private List<SeedEvent> couponEvents(InvestmentHolding h, YearMonth month) {
    ...
    return InterestCalculator.couponDates(t, h.startDate(), month.atEndOfMonth()).stream()
        ...
        .map(date -> new SeedEvent(date, () -> coupon(h, date)))
        .toList();
  }
  ...
  private HoldingRequest request(Long companyId, SeedHolding d) {
  ...
  private static List<SeedHolding> table(String identities, String terms) {
    List<String> left = identities.lines().toList();
    List<String> right = terms.lines().toList();
    List<SeedHolding> rows = new ArrayList<>();
    for (int i = 0; i < left.size(); i++) {
      String termsLine = right.get(i);
      String line = left.get(i) + termsLine.substring(termsLine.indexOf('|'));
      rows.add(SeedHolding.parse(line.split("\\|")));
    }
    return List.copyOf(rows);
  }

  private static LocalDate start(SeedHolding d) {
    LocalDate settlement = LocalDate.parse(d.settlement());
  ...
  /** A dated seed event, executed in date order within its month. */
  private record SeedEvent(LocalDate date, Supplier<Object> action) {}

  /** A seed holding definition. */
  private record SeedHolding(
      String code,
      String description,
      String portfolio,
      InstrumentType type,
    ...
    static SeedHolding parse(String[] fields) {
      Iterator<String> f = List.of(fields).iterator();
      return new SeedHolding(
          f.next(),
          f.next(),
          f.next(),
