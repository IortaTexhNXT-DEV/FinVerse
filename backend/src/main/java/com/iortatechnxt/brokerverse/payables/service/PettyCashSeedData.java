...
/**
 * Seed petty cash activity (called by {@link PayablesSeedData}): every fund is established in
...
public class PettyCashSeedData {
   ...
   */
  public PettyCashSeedData(
      PettyCashFundService funds, PettyCashService pettyCash, SeedActor actor) {
    this.funds = funds;
  ...
  private int month(PettyCashFund fund, int fundIndex, int month) {
    for (int k = 0; k < VOUCHERS_PER_MONTH; k++) {
      ...
      PettyCashDisbursement voucher =
          actor.as(SeedActor.MAKER, () -> pettyCash.disburse(fund.getId(), values));
      actor.as(SeedActor.CHECKER, () -> pettyCash.approveDisbursement(voucher.getId()));
    }
    if (month <= LAST_REIMBURSED_MONTH) {
      ...
      actor.as(SeedActor.CHECKER, () -> pettyCash.approveReimbursement(claim.getId()));
    }
