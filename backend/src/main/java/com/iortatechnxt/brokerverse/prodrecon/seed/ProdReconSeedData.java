package com.iortatechnxt.brokerverse.prodrecon.seed;
...
/**
 * Seed start-up (seed profile only, idempotent), signed in as the reconciliation handler ({@code
 * recon}), once the seed bookings are in the Operations ledger and the other Operations seeds ran
...
@Profile("seed")
@Order(94)
public class ProdReconSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(ProdReconSeedData.class);
   ...
   * @param organization seed company
   * @param users seed sign-in
   */
  public ProdReconSeedData(
  ...
  private void reconcile(Long companyId) {
    ...
    try {
      ...
      LOG.warn("Production reconciliation seed skipped: {}", ex.getMessage());
    }
  }
