...
/**
 * Company, branches and helpers shared by the receivables demo data builders.
 *
 * @param companyId demo company
 * @param headOfficeId head office branch
 * @param branchIds all branches (head office first)
 */
public record DemoContext(Long companyId, Long headOfficeId, List<Long> branchIds) {

  /** Maker of demo receipts. */
  public static final String MAKER = "accountant";

  /** Checker of demo receipts. */
  public static final String CHECKER = "checker";

  /** Finance manager (reconciliation). */
  public static final String MANAGER = "fmanager";

  /** Main demo bank account (BDO current account, PHP). */
  ...
  /** Last value date used by the demo data (all value dates stay within Jan-Sep 2026). */
  ...
  /**
   * Runs an action as a demo user (services record the user as maker / checker).
   *
   * @param username user
   * @param action action
   * @param <T> result type
