...
/**
 * Seed debit notes for the receivables module when no policy administration data exists: a
 * POLICY_ISSUE journal plus a DEBIT open item per note, January to September 2026.
 */
@Component
@Profile("seed")
public class SeedDebitNotes {
  ...
  private static final String MODULE = "RECEIVABLES_SEED";
   ...
   */
  public int ensure(SeedContext ctx) {
    long open =
        openItems.outstanding(ctx.companyId(), SeedContext.LAST_DATE).stream()
  ...
  private void create(
      SeedContext ctx, int n, String partyCode, LocalDate date, String currency, BigDecimal gross) {
    tx.executeWithoutResult(
        status -> {
          ...
          String no = String.format("DN-2026-9%05d", n + 1);
          String key = "SEED-DN:" + (n + 1);
          JournalBatch batch =
              publisher.publish(
                  new BusinessEvent(
                      "POLICY_ISSUE",
