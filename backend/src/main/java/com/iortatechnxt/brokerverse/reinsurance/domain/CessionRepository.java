...
public interface CessionRepository extends JpaRepository<Cession, Long> {
   ...
   */
  @Query(
      "select new com.iortatechnxt.brokerverse.reinsurance.domain.CessionKey(c.policyId,"
  ...
  @Query(
      "select c from Cession c where c.policyId = :policyId"
          + " and c.basis = com.iortatechnxt.brokerverse.reinsurance.domain.CessionBasis.FULL"
