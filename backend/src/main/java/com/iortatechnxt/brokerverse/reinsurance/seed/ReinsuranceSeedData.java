package com.iortatechnxt.brokerverse.reinsurance.seed;
...
/**
 * SEED PROFILE ONLY: creates the reinsurance seed data of the seed company (FVI) at start-up
 * through the services, after the underwriting portfolio ({@code @Order(10)}) and before claims
 * ({@code @Order(20)}), so that every seed claim movement finds the cession of its policy:
 *
 * <ol>
 ...
 *   <li>claim movements posted before any treaty existed (none in the standard seed order),
 *       replayed through the claims port when the claims module is deployed.
 * </ol>
 *
 * <p>The statements of account come later, from {@link ReinsuranceStatementsSeedData}, so that they
 * include the reinsurers' share of the seed claims.
 *
...
@Profile("seed")
@Order(15)
public class ReinsuranceSeedData implements ApplicationRunner {

  static final String MAKER = "reinsurer";
  static final String CHECKER = "fmanager";

  private static final Logger LOG = LoggerFactory.getLogger(ReinsuranceSeedData.class);
  private static final String SEED_COMPANY = "FVI";
  private static final LocalDate YEAR_START = LocalDate.of(SeedTreaties.YEAR, 1, 1);
  private static final LocalDate LAST_POSTING = LocalDate.of(SeedTreaties.YEAR, 9, 22);
  ...
  private void loadIfEmpty(Long companyId) {
    if (treaties.list(companyId).isEmpty()) {
      ...
      LOG.info("Reinsurance seed data already present, skipped");
    }
  }

  /**
   * Creates the seed data (no idempotency check).
   *
  ...
  public List<Treaty> load(Long companyId) {
    ...
    for (TreatyRequest r : SeedTreaties.requests(companyId)) {
    ...
    LOG.info(
        "Reinsurance seed data created: {} treaties, {} cessions, {} FAC placed, {} claim"
            + " movements",
        created.size(),
        run.ceded(),
        placed,
                  ...
                  new FacAssignRequest(
                      List.of(
                          new FacAssignRequest.Line(
                              SeedTreaties.LONDON, LEAD_SHARE, FAC_COMMISSION),
                          new FacAssignRequest.Line(
                              SeedTreaties.ASIA, FOLLOW_SHARE, FAC_COMMISSION)),
                      "Slip placed through " + SeedTreaties.BROKER)));
