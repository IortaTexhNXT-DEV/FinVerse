...
/**
 * SEED PROFILE ONLY: Q1 and Q2 statements of account of every seed treaty, approved, and the Q1
 * fire quota share statement of the local reinsurer settled.
 *
 * <p>Runs after the treaty programme ({@link ReinsuranceSeedData}, {@code @Order(15)}) and the seed
...
@Profile("seed")
@Order(25)
public class ReinsuranceStatementsSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(ReinsuranceStatementsSeedData.class);
  ...
  private static final LocalDate SETTLEMENT_DATE = LocalDate.of(SeedTreaties.YEAR, 7, 31);
   ...
   */
  public ReinsuranceStatementsSeedData(
  ...
  private void loadIfEmpty(Long companyId) {
    if (!treaties.list(companyId).isEmpty() && statements.list(companyId).isEmpty()) {
      LOG.info("Reinsurance seed statements created: {}", load(companyId));
    }
  }

  /**
   * Generates, approves and partly settles the statements of the seed-year treaties (no idempotency
   * check).
   *
  ...
  public int load(Long companyId) {
    ...
    List<Treaty> programme =
        treaties.list(companyId).stream().filter(t -> t.getUwYear() == SeedTreaties.YEAR).toList();
    for (Treaty t : programme) {
      for (int quarter : QUARTERS) {
        LocalDate quarterEnd =
            LocalDate.of(SeedTreaties.YEAR, quarter * MONTHS_PER_QUARTER, 1)
                .plusMonths(1)
                .minusDays(1);
        SoaRequest request =
            new SoaRequest(
        ...
        List<Soa> soas = users.runAs(ReinsuranceSeedData.MAKER, () -> statements.generate(request));
        for (Soa s : soas) {
          users.runAs(ReinsuranceSeedData.CHECKER, () -> statements.approve(s.getId()));
          count++;
        }
      }
    }
  ...
  private void settleOne(Long companyId) {
    statements.list(companyId).stream()
        ...
        .filter(s -> SeedTreaties.LOCAL.equals(s.getParty().getCode()) && s.getQuarter() == 1)
        .filter(s -> s.getBalance().signum() != 0)
        .findFirst()
        .ifPresent(
            s ->
                users.runAs(
                    ReinsuranceSeedData.CHECKER,
                    () ->
                        statements.settle(
