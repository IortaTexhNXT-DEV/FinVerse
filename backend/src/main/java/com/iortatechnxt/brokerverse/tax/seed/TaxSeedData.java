 ...
 * </ul>
 *
 * Idempotent: each calendar entry is only brought forward to its seed state (a missing return is
...
@Profile("seed")
@Order(95)
public class TaxSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(TaxSeedData.class);
  ...
  /**
   * Loads the seed data of a company (idempotent).
   *
  ...
  public void load(Long companyId) {
    ...
    int count = 0;
    for (CalendarEntry e : calendar.calendar(companyId, YEAR, SEED_TODAY)) {
      if (e.tracked() && complete(companyId, e)) {
        count++;
      }
    }
    LOG.info("Tax seed data: {} returns created or completed", count);
    certify(companyId, FIRST_QUARTER);
    certify(companyId, SECOND_QUARTER);
  }

  /**
   * Brings one calendar entry to its seed state: Q1-Q2 PAID, Q3 (ended months and quarters) at
  ...
  /** Seed state of a period: PAID up to June, DRAFT for ended months and the quarters of Q3. */
  private static Optional<ReturnStatus> target(TaxPeriod period) {
    ...
    boolean ended = period.isQuarter() || period.to().isBefore(SEED_TODAY);
  ...
  private void fileAndPay(Long id, LocalDate day) {
    ...
    if (r.getStatus() == ReturnStatus.DRAFT) {
      actor.as(SeedActor.CHECKER, () -> returns.file(id, day, "EFPS-" + r.getReturnNo()));
    }
    actor.as(
        SeedActor.CHECKER,
  ...
  private void certify(Long companyId, int quarter) {
    ...
    try {
      actor.as(SeedActor.MAKER, () -> certificates.generate(companyId, YEAR, quarter));
