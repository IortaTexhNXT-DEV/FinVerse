...
/**
 * Seed quotations in every status (draft, pending, approved, rejected, converted, expired) and a
 * USD marine open cover with monthly shipment certificates.
 */
final class SeedQuotationsAndCovers {
  ...
  private static QuotationRequest request(
    ...
    return new QuotationRequest(
        ...
        plan.clientCode(),
        SeedCatalog.INSURED.get(SeedCatalog.CLIENTS.indexOf(plan.clientCode())),
  ...
  /** Target status of a seed quotation. */
  enum Target {
    DRAFT,
    PENDING,
    APPROVED,
