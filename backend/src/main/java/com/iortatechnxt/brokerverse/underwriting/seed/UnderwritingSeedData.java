...
import static com.iortatechnxt.brokerverse.underwriting.seed.SeedPolicyGenerator.CHECKER;
import static com.iortatechnxt.brokerverse.underwriting.seed.SeedPolicyGenerator.MAKER;
...
/**
 * SEED PROFILE ONLY: creates the underwriting seed portfolio of the seed company (FVI) at start-up
...
@Profile("seed")
@Order(10)
public class UnderwritingSeedData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(UnderwritingSeedData.class);
  ...
  private final SeedPolicyGenerator policies;
  private final SeedQuotationsAndCovers quotationsAndCovers;
  ...
  public UnderwritingSeedData(
    ...
    this.users = users;
    this.policies = new SeedPolicyGenerator(users, policyService, approvals, endorsements);
    this.quotationsAndCovers =
        new SeedQuotationsAndCovers(users, quotations, openCovers, policyService, approvals);
  }
  ...
  private void loadIfEmpty(Long companyId) {
    if (policyRepository.countByCompanyId(companyId) > 0) {
      LOG.info("Underwriting seed data already present, skipped");
    } else {
      load(companyId);
    }
  }
  ...
  /**
   * Creates the seed portfolio (no idempotency check).
   *
  ...
  public List<Policy> load(Long companyId) {
    ...
    LOG.info(
        "Underwriting seed data created: {} products, {} policies, {} quotations, 1 open cover",
        byCode.size(),
        created.size(),
        quotations);
    return created;
  ...
  private Map<String, Product> ensureProducts(Long companyId) {
    ...
    for (ProductProfile profile : SeedCatalog.PRODUCTS) {
      if (!byCode.containsKey(profile.code())) {
        Product created =
            users.runAs(
                MAKER, () -> products.create(SeedCatalog.productRequest(companyId, profile)));
