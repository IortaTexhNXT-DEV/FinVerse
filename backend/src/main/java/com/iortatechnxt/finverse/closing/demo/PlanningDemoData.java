...
/**
 * Demo data of budgeting, inter-company, consolidation and FX revaluation (demo profile only,
 * idempotent, runs after the operational demo data): subsidiary journals and inter-company
...
@Profile("demo")
@Order(60)
public class PlanningDemoData implements ApplicationRunner {

  private static final Logger LOG = LoggerFactory.getLogger(PlanningDemoData.class);
  ...
  private final DemoSubsidiaryData subsidiary;
  private final DemoBudgetData budget;
   ...
   * @param userDetails demo users (maker and checker)
   */
  public PlanningDemoData(
    ...
    this.groups = groups;
    this.runs = runs;
    this.users = new DemoUsers(userDetails);
    this.subsidiary = new DemoSubsidiaryData(journals, intercompany, organization);
    this.budget = new DemoBudgetData(budgets, budgetLines, users);
  }
  ...
  public void run(ApplicationArguments args) {
    ...
    if (parent.isEmpty() || sub.isEmpty()) {
      LOG.info("Planning demo data skipped: demo companies {} / {} not found", PARENT, SUBSIDIARY);
      return;
    }
    ...
    LOG.info("Planning and closing demo data ready");
  }
  ...
  private void step(String name, Runnable action) {
    try {
      ...
      LOG.warn("Planning demo data step '{}' skipped: {}", name, ex.getMessage());
    }
  }
