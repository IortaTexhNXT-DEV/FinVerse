...
public interface CessionLineRepository extends JpaRepository<CessionLine, Long> {
   ...
   */
  @Query(
      "select new com.iortatechnxt.finverse.reinsurance.domain.AmountPair(sum(l.basePremium),"
  ...
  @Query(
      "select l from CessionLine l join fetch l.cession c where c.companyId = :companyId"
          + " and l.layer = com.iortatechnxt.finverse.reinsurance.domain.RiLayer.FAC"
