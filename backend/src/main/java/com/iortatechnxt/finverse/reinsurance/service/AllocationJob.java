 ...
 * ({@code finverse.jobs.ri-allocation-cron}); the reinsurance screen runs it for a chosen period.
 */
@Component
public class AllocationJob implements ManagedJob {
   ...
   * @param cron schedule ({@code finverse.jobs.ri-allocation-cron}, "-" = manual only)
   */
  public AllocationJob(
      AllocationRunService runs,
      OrganizationService organization,
      @Value("${finverse.jobs.ri-allocation-cron:-}") String cron) {
    this.runs = runs;
