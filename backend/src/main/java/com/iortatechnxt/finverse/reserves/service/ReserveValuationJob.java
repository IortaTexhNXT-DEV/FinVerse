 ...
 * by default: set {@code finverse.jobs.reserve-valuation-cron} (e.g. {@code 0 0 2 1 * *}) to
...
public class ReserveValuationJob implements ManagedJob {
   ...
   * @param cron schedule ({@code finverse.jobs.reserve-valuation-cron}, "-" = manual only)
   */
  public ReserveValuationJob(
      ValuationRunService runs,
      OrganizationService organization,
      SystemParameterService parameters,
      @Value("${finverse.jobs.reserve-valuation-cron:-}") String cron) {
    this.runs = runs;
