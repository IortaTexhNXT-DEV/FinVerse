...
import com.iortatechnxt.finverse.security.domain.RoleRepository;
 ...
 *
 * <p>The password comes from {@code FINVERSE_ADMIN_INITIAL_PASSWORD}; nothing is created when it is
 * absent, so no default credentials ever exist in production.
 */
@Component
public class AdminBootstrap implements ApplicationRunner {
  ...
  public AdminBootstrap(
      ...
      @Value("${FINVERSE_ADMIN_USERNAME:sysadmin}") String username,
      @Value("${FINVERSE_ADMIN_INITIAL_PASSWORD:}") String initialPassword) {
    this.users = users;
    this.roles = roles;
  ...
  public void run(ApplicationArguments args) {
    ...
    if (initialPassword == null || initialPassword.isBlank()) {
      LOG.warn(
          "No users exist and FINVERSE_ADMIN_INITIAL_PASSWORD is not set; no administrator created");
    } else {
      createAdministrator();
    }
  }
