...
/**
 * Generates the demo policy portfolio (January - September 2026) through the underwriting services:
 ...
 * endorsements.
 */
final class DemoPolicyGenerator {
  ...
  private static final int PRODUCTS = DemoCatalog.PRODUCTS.size();
  ...
  List<Policy> generate(Long companyId, Map<String, Long> branches, Map<String, Product> products) {
    ...
    for (int i = 0; i < DemoPolicyRequests.POLICY_COUNT; i++) {
      ProductProfile profile = DemoCatalog.PRODUCTS.get(i % PRODUCTS);
      PolicyRequest request =
          DemoPolicyRequests.request(i, companyId, branches, products.get(profile.code()), profile);
  ...
  private void endorse(int i, Policy policy) {
    ...
    if (i % DemoPolicyRequests.SHORT_TERM_CYCLE == 1
