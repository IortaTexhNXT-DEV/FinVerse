package com.iortatechnxt.finverse.underwriting.demo;

import static com.iortatechnxt.finverse.underwriting.demo.DemoCatalog.draw;
...
import com.iortatechnxt.finverse.underwriting.demo.DemoCatalog.ProductProfile;
...
/**
 * Builds the deterministic policy requests of the demo portfolio: client, channel, period, share,
 * discount / loading and risks (sums insured and rates within each product's profile).
 */
final class DemoPolicyRequests {

  /** Number of demo policies. */
  ...
  private DemoPolicyRequests() {}

  /**
   * Request of demo policy number {@code i}.
   *
  ...
  static PolicyRequest request(
    ...
    String clientCode = DemoCatalog.CLIENTS.get(client);
    ...
    return new PolicyRequest(
        companyId,
        branches.get(DemoCatalog.BRANCHES.get(draw(i, SALT_BRANCH) % DemoCatalog.BRANCHES.size())),
        product.getId(),
        clientCode,
        DemoCatalog.INSURED.get(client),
        source,
  ...
  private static List<RiskRequest> risks(int i, ProductProfile p, boolean usd) {
    ...
    String zone = DemoCatalog.ZONES.get(draw(i, SALT_ZONE) % DemoCatalog.ZONES.size());
    String occupation = DemoCatalog.OCCUPATIONS.get(i % DemoCatalog.OCCUPATIONS.size());
