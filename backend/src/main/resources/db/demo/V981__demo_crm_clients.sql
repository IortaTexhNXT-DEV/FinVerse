-- =====================================================================================
-- iNXT BrokerVerse - V981 Demo CRM clients (DEMO ONLY - never load in production).
...
create temporary table demo_client (
...
insert into demo_client values
('PR-2026-000001', 'CL-2026-000001', 'CONFIRMED', 'INDIVIDUAL', 'Santos', 'Maria Clara', 'Reyes', null,
 date '1984-03-12', '201-555-101-000', 'PASSPORT', 'P5551010A', 'maria.santos@demo-client.ph', '09175550101',
...
('PR-2026-000002', 'CL-2026-000002', 'CONFIRMED', 'INDIVIDUAL', 'Reyes', 'Jose Miguel', 'Lopez', null,
 date '1979-11-02', '201-555-102-000', 'DRIVERS_LICENSE', 'N01-55-102030', 'jm.reyes@demo-client.ph', '09185550102',
...
('PR-2026-000005', 'CL-2026-000005', 'CONFIRMED', 'INDIVIDUAL', 'Garcia', 'Antonio Luis', 'Dizon', null,
 date '1990-07-21', '201-555-105-000', 'UMID', '0111-5550105-3', 'antonio.garcia@demo-client.ph', '09175550105',
...
('PR-2026-000006', 'CL-2026-000006', 'CONFIRMED', 'INDIVIDUAL', 'Villanueva', 'Carmela Isabel', 'Santos', null,
 date '1972-01-30', '201-555-106-000', 'PHILSYS', '5555-0106-2026-0001', 'carmela.v@demo-client.ph', '09185550106',
...
('PR-2026-000007', null, 'PROSPECT', 'INDIVIDUAL', 'Mendoza', 'Rafael Jose', 'Castro', null,
 date '1988-05-05', '201-555-107-000', 'PASSPORT', 'P5551070B', 'rafael.mendoza@demo-client.ph', '09175550107',
...
('PR-2026-000011', null, 'PROSPECT', 'INDIVIDUAL', 'Cruz', 'Benjamin Tomas', 'Ramos', null,
 date '1965-09-09', '201-555-111-000', 'SSS', '33-5550111-1', 'ben.cruz@demo-client.ph', '09175550111',
...
('PR-2026-000012', null, 'INACTIVE', 'INDIVIDUAL', 'Lim', 'Stephanie Ann', 'Go', null,
 date '1993-12-24', '201-555-112-000', 'PASSPORT', 'P5551120C', 'stephanie.lim@demo-client.ph', '09175550112',
...
from crm_client c
join demo_client d on d.prospect_code = c.prospect_code;
...
where w.workflow_code = 'NB_CLIENT' and w.created_by = 'ao'
  and w.reference in (select coalesce(client_code, prospect_code) from demo_client);
...
cross join (select convert_to('%PDF-1.4' || chr(10) || '% BrokerVerse demo KYC document' || chr(10) || '%%EOF', 'UTF8') as content) b
where d.stage in ('KYC_REVIEW', 'KYC_VERIFIED', 'CONFIRMED');

insert into doc_attachment_content (attachment_id, content)
select a.id, convert_to('%PDF-1.4' || chr(10) || '% BrokerVerse demo KYC document' || chr(10) || '%%EOF', 'UTF8')
...
-- Number series continue after the demo codes.
