-- =====================================================================================
-- iNXT BrokerVerse - V988 Demo booking (demo profile only).
--   * Demo GL accounts of the broker booking entry (OPERATIONS_DESIGN section 5 numbering),
...
--   * Demo accounting rule of BROKER_BOOKING (real accounts come from Comptrollership, OQ07).
--   * One demo incentive rule (Q33 parked) and one demo auto-book rule (BRNB.076).
--   * Seven accounts in POLICY_ISSUED for demo clients CL-2026-000001..006 (V981). The demo
--     seeder (booking.demo.BookingDemoData) books four of them through the real booking
...
-- ---------- Demo GL accounts ---------------------------------------------------------------
create temporary table bkg_demo_gl (
...
insert into bkg_demo_gl values
...
from org_company c
cross join (select * from bkg_demo_gl order by seq) g
where c.code = 'FVI'
...
-- ---------- Demo accounting rule of BROKER_BOOKING (OPERATIONS_DESIGN section 5, row 0) ------
insert into acc_rule (company_id, event_type, name, priority, effective_from, record_status,
    authorized_by, authorized_at, created_at, created_by)
select id, 'BROKER_BOOKING', 'Broker booking - premium receivable, DTIP and commission (demo)', 100,
...
select id, 'MTR10', 'CBG', null, date '2026-01-01', date '2026-12-31', true,
       'Demo: CBG motor MTR10 campaign 2026 (qualification rules pending, Q33)', now(), 'SYSTEM'
from org_company where code = 'FVI';

insert into bkg_auto_book_rule (company_id, product_code, market_segment, enabled, description,
    created_at, created_by)
select id, 'PAR08', 'CBG', true, 'Demo: CBG fire PAR08 is booked by the end-of-day batch', now(), 'SYSTEM'
from org_company where code = 'FVI';

-- ---------- Demo accounts in POLICY_ISSUED ---------------------------------------------------
create temporary table bkg_demo_account (
...
insert into bkg_demo_account values
...
('ARN-2026-940002', 'CL-2026-000005', 'PAR01', 'PROPERTY', 'FIRE_LIGHTNING', 'CBG', 'INS-MGIC', 'MKT',
 ...
 'PROPERTY_LOCATION', '9 Booking Demo Street, Ortigas Center', 0.25, 'MGIC-FI-2026-98802', date '2026-09-08',
...
('ARN-2026-940006', 'CL-2026-000006', 'PAR08', 'PROPERTY', 'FIRE_LIGHTNING', 'CBG', 'INS-LAC', 'QC',
 ...
 'PROPERTY_LOCATION', '12 Booking Demo Road, Lahug', 0.25, 'LAC-FI-2026-98806', date '2026-09-18',
...
('ARN-2026-940007', 'CL-2026-000005', 'PAR01', 'PROPERTY', 'FIRE_LIGHTNING', 'CBG', 'INS-MGIC', 'MKT',
 ...
 4768.75, 22268.75, 25, 4375.00, 525.00, 'VIA_BDOI', 'PAID', 'NCR', 'CBG-NCR', 'T-CBG1', 'ao', 'NB-CBG-M',
 'PROPERTY_LOCATION', '27 Multi-Year Demo Avenue, Kapitolyo', 0.25,
...
from bkg_demo_account d join acc_account a on a.arn = d.arn
...
-- Queued for the end-of-day batch by the demo auto-book rule (PAR08 CBG).
