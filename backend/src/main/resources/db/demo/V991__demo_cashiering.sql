-- =====================================================================================
-- iNXT BrokerVerse - V991 Demo Cashiering (demo profile only). DEMO ONLY.
--   * Demo accounting rules of the Cashiering events (OPERATIONS_DESIGN section 5, rows 1-11 and
--     14-15). The real accounts come from Comptrollership (OQ07).
--   * Demo account 4190 Other Operating Income for "Others" official receipts.
--   * Receipt series (BIR ATP ranges parked, OQ05): AR for Head Office and Cebu, OR for Head
--     Office only (CSHID.006), authorized.
--   * Two check pick-up requests from Collection (CSHID.009) on demo accounts.
--   The payments, receipts, applications and 2307 tags of the storyline are posted at start-up
--   by cashiering.demo.CashieringDemoData through the real services, after the ledger replay.
-- =====================================================================================
...
from org_company c
cross join (values
    ('OPS_AR_RECEIPT', 'AR - cash in to unapplied collections (demo)'),
    ('OPS_PAYMENT_APPLY', 'Payment application by PR component (demo)'),
    ('OPS_CWT_RECLASS', 'PR to PR2307 reclass (demo)'),
    ('OPS_CWT_DTIP_OFFSET', 'PR2307 offset against DTIP (demo)'),
    ('OPS_EXCESS_TO_OVERAGES', 'Minimal excess to AP overages (demo)'),
    ('OPS_MINIMAL_BALANCE_REVERSAL', 'PR minimal balance reversal (demo)'),
    ('OPS_UNAPPLIED_REFUND', 'Unapplied refund payable (demo)'),
    ('OPS_UNAPPLIED_RECLASS', 'Unapplied reclass or transfer (demo)'),
    ('OPS_RECEIPT_REINSTATE', 'Receipt reinstatement (demo)'),
    ('OPS_AR_INSURANCE_RECEIPT', 'Insurer non-premium receipt to AR Insurer (demo)'),
    ('OPS_OR_ISSUE', 'Official receipt - income, VAT and CWT (demo)')
) as e(code, name)
where c.code = 'FVI';
...
join (values
    ('HO', 'AR', 'ATP-DEMO-AR-HO', 'AR-HO-', 1, 999999, 500),
    ('CEB', 'AR', 'ATP-DEMO-AR-CEB', 'AR-CEB-', 1, 99999, 200),
    ('HO', 'OR', 'ATP-DEMO-OR-HO', 'OR-HO-', 100001, 100600, 100)
...
-- ---------- Collection accounts of the demo chart ---------------------------------------------
