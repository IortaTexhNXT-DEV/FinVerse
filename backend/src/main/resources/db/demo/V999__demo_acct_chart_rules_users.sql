-- =====================================================================================
-- iNXT BrokerVerse - V999 Demo Accounting, Disbursement and ACSL (BRD-5) chart, rules and users
-- (demo profile only; password for all users: Brokerverse@2026). DEMO ONLY - never load in
-- production.
--   * Demo chart (docs/architecture/ACCOUNTING_DISBURSEMENT_DESIGN.md section 3): the Operations
--     placeholder accounts take the names BDOI uses in its reports, and the accounts the BRD
--     names without a demo account are added. The real chart is uploaded by FRBS (FRBS 2.3.1,
--     AQ01); no code refers to an account code.
--   * Demo accounting rules of the event types seeded in V890 (design section 6). The real rules
--     come from Comptrollership (AQ02).
--   * Demo users of the BRD-5 roles of V890 (design section 8.2). disb (DISBURSEMENT) exists
--     since V990 and received its new grants through its role.
--   The demo transactions (payees, DVs, requests, cases, service-fee runs) are created at start-up
--   by the Java demo runners of the modules (A1 waves), after the Operations storyline.
-- =====================================================================================

-- ---------- Demo chart: BDOI names of the Operations accounts (design section 3) --------------
...
-- ---------- Demo chart: accounts named by the BRD (design section 3) ---------------------------
create temporary table acct_demo_gl (
...
insert into acct_demo_gl values
...
from org_company c
cross join (select * from acct_demo_gl order by seq) g
where c.code = 'FVI'
  and not exists (select 1 from coa_account a where a.company_id = c.id and a.code = g.code)
order by g.seq;

-- ---------- Demo accounting rules of the BRD-5 events (design section 6) -----------------------
...
from org_company c
cross join (values
    ('DISB_VOUCHER', 'Disbursement voucher - payable settled by type (demo)'),
    ('DISB_CHECK_NEGOTIATED', 'Check negotiated - checks outstanding cleared (demo)'),
    ('DISB_CHECK_STALE', 'Check staled - to miscellaneous liability (demo)'),
    ('DISB_FUND_TRANSFER', 'Account funding - bank to bank (demo)'),
    ('TAX_CWT_CERT_RECEIVED', 'Insurer 2307 received - AR-BIR on hand (demo)'),
    ('OPS_REMIT_CPC2', 'CPC2 incentive deducted from the remittance (demo)'),
    ('OPS_REMIT_DEDUCTION', 'AR insurer''s refund deducted from the remittance (demo)'),
    ('FRBS_SERVICE_FEE_ACCRUE', 'Service fee accrual (demo)'),
    ('PRQ_CA_LIQUIDATION', 'Cash advance liquidation (demo)')
) as e(code, name)
where c.code = 'FVI'
...
-- ---------- Demo users of the BRD-5 roles (design section 8.2) --------------------------------
...
from (values ('glofficer', 'Gloria GL Officer',            'glofficer@brokerverse-demo.ph'),
             ('gltl',      'Gerardo GL Team Lead',         'gltl@brokerverse-demo.ph'),
             ('glhead',    'Graciela GL Section Head',     'glhead@brokerverse-demo.ph'),
             ('disbtl',    'Diego Disbursement Lead',      'disbtl@brokerverse-demo.ph'),
             ('disbtl2',   'Dolores Disbursement Verifier', 'disbtl2@brokerverse-demo.ph'),
             ('disbappr',  'Daniel Disbursement Approver', 'disbappr@brokerverse-demo.ph'),
             ('disbappr2', 'Divina Disbursement Approver', 'disbappr2@brokerverse-demo.ph'),
             ('mktao',     'Marco Marketing Processor',    'mktao@brokerverse-demo.ph'),
             ('mktrev',    'Maricel Marketing Reviewer',   'mktrev@brokerverse-demo.ph'),
             ('mktappr',   'Manolo Marketing Approver',    'mktappr@brokerverse-demo.ph'),
             ('hrappr',    'Helena HR Approver',           'hrappr@brokerverse-demo.ph'),
             ('acsl',      'Arturo ACSL Processor',        'acsl@brokerverse-demo.ph'),
             ('acsltl',    'Andrea ACSL Team Leader',      'acsltl@brokerverse-demo.ph'),
             ('acslhead',  'Antonio ACSL Head',            'acslhead@brokerverse-demo.ph'))
