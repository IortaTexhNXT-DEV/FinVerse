...
--   Configuration: one ACTIVE version of every type for the seed company FVI (test data TD-SS-02):
...
--   one PENDING SLA version made by compoff for compchk's My Approvals. All values are seed values
...
from (values ('compoff',       'Carmela Compliance Officer',   'compoff@brokerverse-seed.ph'),
             ('compchk',       'Cesar Compliance Checker',     'compchk@brokerverse-seed.ph'),
             ('compdual',      'Dolores Compliance Maker-Checker', 'compdual@brokerverse-seed.ph'),
             ('ucc',           'Ursula Unit Compliance Coordinator', 'ucc@brokerverse-seed.ph'),
             ('investigator',  'Ivan Investigator',            'investigator@brokerverse-seed.ph'),
             ('investigator2', 'Isabel Investigator',          'investigator2@brokerverse-seed.ph'),
             ('scrapprover',   'Samuel Screening Approver',    'scrapprover@brokerverse-seed.ph'),
             ('amlcom1',       'Amparo AML Committee',         'amlcom1@brokerverse-seed.ph'),
             ('amlcom2',       'Alfredo AML Committee',        'amlcom2@brokerverse-seed.ph'))
...
select c.id, t.config_type, t.scope, 1, 'ACTIVE', date '2026-01-01',
       'Initial seed configuration (values to be confirmed by Compliance)', 'compoff',
...
from scr_config_version v
join org_company c on c.id = v.company_id and c.code = 'FVI'
join (values ('KYC_REVIEW', 'KYC review (seed)'), ('TRANSACTION_REVIEW', 'Transaction review (seed)'),
             ('EDD', 'Enhanced due diligence (seed)'), ('STR', 'Suspicious transaction report (seed)'))
...
select c.id, 'SLA_MATRIX', '', 2, 'PENDING', date '2026-10-01',
       'Shorter investigation SLA (seed proposal)', v1.id, 'compoff', now(),
