...
--     reasons   seed STR reason codes RSN01-RSN03 in SCR_STR_REASON, so that an STR can be
--               completed in the seed; the AMLC reason codes replace them when BDOI supplies the
--               format (SQ09). Production keeps the list empty until then.
--     cases     one case in every stage of SCR_CASE for the seed company FVI: NEW, INVESTIGATION
...
--   Clients are the seed clients of V981; the configuration versions those of V1950.
-- =====================================================================================
...
from (values ('amlcom3', 'Arturo AML Committee', 'amlcom3@brokerverse-seed.ph'),
             ('amlcom4', 'Aurora AML Committee', 'amlcom4@brokerverse-seed.ph'),
             ('amlcom5', 'Anselmo AML Committee', 'amlcom5@brokerverse-seed.ph'),
             ('scrdual', 'Sofia Investigator-Approver', 'scrdual@brokerverse-seed.ph'))
...
-- ---------- Seed STR reasons (SQ09: the AMLC codes replace them) --------------------------------
...
select v.type_code, v.code, v.label, v.sort_order, null, date '2020-01-01', 'ACTIVE', 'SYSTEM', now(), now(),
       'SYSTEM'
from (values
    ('SCR_STR_REASON', 'RSN01', 'Placeholder: transaction with a listed person (to be replaced, SQ09)', 10),
    ('SCR_STR_REASON', 'RSN02', 'Placeholder: no apparent economic purpose (to be replaced, SQ09)', 20),
    ('SCR_STR_REASON', 'RSN03', 'Placeholder: amount not commensurate with the profile (to be replaced, SQ09)', 30)
...
insert into wf_case_history (case_id, from_stage, to_stage, action, reason_code, comment, actor, automatic,
    occurred_at)
select w.id, 'NEW', k.stage, 'seed', null, 'Seed case in ' || k.stage, 'SYSTEM', true, k.stage_entered_at
from wf_case w join scr_case k on k.work_case_id = w.id
...
select k.id, 'CREATED', 1, null, null, null, null, null, 'Opened by ' || k.trigger_code || ' (seed)', 'SYSTEM',
...
from scr_case k
join (values ('SCR-2026-000008', 'STR-2026-000001', 'RSN02', 'APPROVED'),
             ('SCR-2026-000009', 'STR-2026-000002', 'RSN01', 'EXTRACTED')) as s(case_no, str_no, reasons, status)
...
insert into scr_str_transaction (str_id, reference, txn_date, amount, currency, txn_type, description,
    created_at, created_by)
select s.id, 'OR-2026-900' || right(s.str_no, 3), (now() - interval '20 days')::date, 125000.00, 'PHP', 'RECEIPT',
       'Premium paid in cash (seed)', now(), 'SYSTEM'
from scr_str s where s.str_no in ('STR-2026-000001', 'STR-2026-000002');

-- ---------- Number series continue after the seed numbers ------------------------------------
