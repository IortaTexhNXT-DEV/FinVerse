...
--     seeds its own event types and seed rules (V764-V789, V991-V995).
...
--   booking.seed.BookingSeedData when it is committed, and OpsLedgerSeedReplay replays the
...
from (values ('cashier',  'Carmela Cashier',           'cashier@brokerverse-seed.ph',  'HO'),
             ('cashbr',   'Benjie Branch Cashier',     'cashbr@brokerverse-seed.ph',   'CEB'),
             ('cashtl',   'Cecilia Cashiering Lead',   'cashtl@brokerverse-seed.ph',   'HO'),
             ('remit',    'Rafael Remittance',         'remit@brokerverse-seed.ph',    'HO'),
             ('remittl',  'Rosario Remittance Lead',   'remittl@brokerverse-seed.ph',  'HO'),
             ('recon',    'Regina Reconciliation',     'recon@brokerverse-seed.ph',    'HO'),
             ('adjtl',    'Alfredo Adjustment Lead',   'adjtl@brokerverse-seed.ph',    'HO'),
             ('commrec',  'Corazon Commission',        'commrec@brokerverse-seed.ph',  'HO'),
             ('commtl',   'Cristina Commission Lead',  'commtl@brokerverse-seed.ph',   'HO'),
             ('mktcoll',  'Miguel Marketing Collection', 'mktcoll@brokerverse-seed.ph', 'HO'),
             ('comptrol', 'Consuelo Comptroller',      'comptrol@brokerverse-seed.ph', 'HO'),
             ('disb',     'Dante Disbursement',        'disb@brokerverse-seed.ph',     'HO'))
...
-- ---------- Seed GL accounts of the Operations events --------------------------------------
create temporary table ops_seed_gl (
...
insert into ops_seed_gl values
...
from org_company c
cross join (select * from ops_seed_gl order by seq) g
where c.code = 'FVI'
  and not exists (select 1 from coa_account a where a.company_id = c.id and a.code = g.code)
order by g.seq;

-- ---------- BOOK rates (2 decimals) from the seed SPOT rates -------------------------------
