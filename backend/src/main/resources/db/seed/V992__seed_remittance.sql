-- =====================================================================================
-- iNXT BrokerVerse - V992 Seed Remittance (seed profile only). SEED DATA ONLY.
--   * Seed accounting rules of OPS_REMITTANCE and OPS_REMIT_INCENTIVE (OPERATIONS_DESIGN 5
--     rows 12 and 13) on the seed chart; the real accounts come from Comptrollership (OQ07).
...
--     are parked (OQ23).
--   The seed storyline itself (hold, special remittance request, extraction run) is created at
--   start-up by remittance.seed.RemittanceSeedData on the invoices of the booked seed accounts.
-- =====================================================================================

insert into acc_rule (company_id, event_type, name, priority, effective_from, record_status,
    authorized_by, authorized_at, created_at, created_by)
select id, 'OPS_REMITTANCE', 'Remittance to insurer - DTIP, CWT, commission and net due (seed)', 100,
...
insert into acc_rule (company_id, event_type, name, priority, effective_from, record_status,
    authorized_by, authorized_at, created_at, created_by)
select id, 'OPS_REMIT_INCENTIVE', 'Early remittance incentive (seed)', 100,
       date '2020-01-01', 'ACTIVE', 'SYSTEM', now(), now(), 'SYSTEM'
from org_company where code = 'FVI';
