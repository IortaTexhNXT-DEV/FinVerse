-- =====================================================================================
-- iNXT BrokerVerse - V993 Seed production reconciliation (seed profile only). SEED DATA ONLY.
--   * Seed values of the lists BDOI has still to provide (OQ31): company concerned
--     (PRCID.016, with "Others") and dispositions (PRCID.038/039). Production keeps them empty.
--   * Seed extraction schedules for the seed insurers (frequency parked, OQ29): monthly on
--     the 5th, not sent automatically.
--   The cycle itself is seed data of prodrecon.seed.ProdReconSeedData: after the seed
