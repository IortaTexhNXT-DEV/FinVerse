-- =====================================================================================
-- iNXT BrokerVerse - V994 Seed Adjustment (seed profile only). SEED DATA ONLY.
--   * Seed GL account 4190 Other Income (credit balances of the minimal balance file,
--     OPERATIONS_DESIGN section 5 row 21), only where missing.
--   * Seed accounting rules of the Adjustment events (rows 18, 21 and the commission-only
...
--   The seed requests themselves are raised at start-up on the booked seed invoices by
--   adjustment.seed.AdjustmentSeedData, through the real services.
-- =====================================================================================
...
from org_company c
cross join (values
    ('OPS_AR_INSURER_SETUP', 'AR Insurer set-up after a remitted decrease (seed)'),
    ('OPS_WRITE_OFF', 'Minimal balance write-off / credit (seed)'),
    ('OPS_ADJ_COMMISSION', 'Commission adjustment without premium change (seed)')
