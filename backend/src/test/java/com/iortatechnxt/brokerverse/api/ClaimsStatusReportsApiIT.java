...
import com.iortatechnxt.brokerverse.brokerclaims.BrokerClaimFixtures;
 ...
 * actions, diary, home, worklist, reassignment, loss experience and Claims Setup, with the rights
 * of the FRS access matrix (officers, TL, TH, UH, Marketing).
 */
@IntegrationTest
class ClaimsStatusReportsApiIT {
  ...
  void theRightsFollowTheAccessMatrix() throws Exception {
    Long id =
        fixtures.recorded(
            fixtures.spec("clmofficer", BrokerClaimFixtures.today()), "NEW_COMPLETE_DOCS");
    String claim = BASE + id;
    ...
    api.doPost(
            "clmofficer",
            BASE + "reassign" + q(),
            Map.of("claimIds", List.of(id), "handler", "clmtl"))
        .andExpect(status().isForbidden());
    api.doGet("clmth", BASE + "setup/matrix").andExpect(status().isForbidden());
    api.doPost(
            "clmofficer", claim + "/status" + q(), Map.of("statusCode", "INSURER_CHECK_ISSUANCE"))
        .andExpect(status().is4xxClientError())
        .andExpect(
            jsonPath("$.detail")
                .value("You are not allowed to set the status For Insurer's Issuance of Check"));
    api.doPost(
            "clmtl",
            claim + "/status" + q(),
            Map.of("statusCode", "INSURER_REVIEW", "remark", "Sent"))
        .andExpect(status().isOk())
    ...
    api.doPost(
            "clmofficer",
            claim + "/diary" + q(),
            Map.of("entryType", "CALL", "text", "Called the insurer"))
