 ...
 * by insurer claim number, insurer claim numbers and insurer location references, each row validated
 * before it is applied; and the demo storyline of wave CL1-A run through the services.
 */
@IntegrationTest
class ClaimsBulkIT {
  ...
  private BulkContext context() {
    return new BulkContext(fx.company(), "BLK-TEST-" + System.nanoTime(), LocalDate.now(), Map.of());
  }
        ...
        () ->
            recording.record(
                ...
                ClaimsFixtures.request(
                    invoice.getArn(), ClaimsFixtures.loss("FIRE"), List.of(new LocationPick(1, null)), List.of())));
  }
  ...
  void insurerClaimNumbersAndUpdatesLoadByUpload() {
    ...
    BulkRow numberRow =
        row(1, "Claim No.", claim.getClaimNo(), "Insurer", LEAD, "Insurer Claim No.", number, "Reported To Insurer On", today);
    assertThat(numberHandler.validate(numberRow, ctx)).isEmpty();
    assertThat(numberHandler.duplicateKey(numberRow)).contains(number);
    assertThat(numberHandler.validate(row(2, "Claim No.", "BCL-0000-000000", "Insurer", LEAD, "Insurer Claim No.", "X", "Reported To Insurer On", "2999-01-01"), ctx))
        .hasSize(2);
    ...
    BulkRow byNumber =
        row(1, "Insurer", LEAD, "Insurer Claim No.", number, "Update Date", today, "Source", "EMAIL", "Remarks", "Bordereau line");
    assertThat(updateHandler.validate(byNumber, ctx)).isEmpty();
    BulkRow unmatched =
        row(37, "Insurer", LEAD, "Insurer Claim No.", "C-INSB-0000", "Update Date", today, "Source", "EMAIL", "Remarks", "x");
    assertThat(updateHandler.validate(unmatched, ctx))
        .containsExactly("No claim found for " + LEAD + " C-INSB-0000");
    assertThat(updateHandler.validate(row(3, "Update Date", today, "Source", "NOPE", "Remarks", "x"), ctx))
        .hasSize(2);
    ...
    as.run(
        ...
        () ->
            updateHandler.commit(
                row(4, "Claim No.", claim.getClaimNo(), "Update Date", today, "Source", "LETTER", "Remarks", "Letter"), ctx));
  ...
  void locationReferencesLoadByUpload() {
    ...
    BulkContext ctx = context();
    BulkRow good =
        row(1, "ARN", invoice.getArn(), "Item No.", "2", "Insurer", LEAD, "Insurer Location Ref", "UPL-LOC-2", "Effective From", "2026-09-01");
    assertThat(refHandler.validate(good, ctx)).isEmpty();
    assertThat(refHandler.validate(row(2, "ARN", "ARN-NONE", "Item No.", "1"), ctx)).hasSize(1);
    assertThat(refHandler.validate(row(3, "ARN", invoice.getArn(), "Item No.", "9"), ctx)).hasSize(1);
  ...
  void theDemoStorylineRecordsClaimsThroughTheServices() {
    ...
    assertThat(
            jdbc.queryForList(
                "select distinct created_by from bcl_claim where arn like 'ARN-2026-9400%'", String.class))
        .isNotEmpty()
