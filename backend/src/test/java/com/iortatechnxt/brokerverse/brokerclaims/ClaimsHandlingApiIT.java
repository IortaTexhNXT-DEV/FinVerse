...
class ClaimsHandlingApiIT {
  ...
  void theClaimEndpointsRecordReadAndWorkAClaim() throws Exception {
    ...
    api.doGet(OFFICER, BASE + "/covers" + c() + "&by=POLICY_NO&q=" + "FI-").andExpect(status().isOk());
    api.doGet(OFFICER, BASE + "/covers" + c() + "&q=MC").andExpect(status().isUnprocessableEntity());
    api.doGet(OFFICER, BASE + "/covers/" + arn + c())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items.length()").value(2))
        .andExpect(jsonPath("$.invoices[0].invoiceNo").value(invoice.getInvoiceNo()));
    api.doGet(OFFICER, BASE + "/covers/" + arn + "/claim-draft" + c() + "&policyYear=1&lossDate=" + LocalDate.now())
        .andExpect(status().isOk())
    ...
    api.doPost(OFFICER, one + "/insurers/" + lead + "/number" + c(), Map.of("insurerClaimNo", "API-" + id, "confirmReuse", false))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].insurerClaimNo").value("API-" + id));
    api.doPost(OFFICER, one + "/insurers/" + lead + "/reserve" + c(), Map.of("amount", 1000, "reason", "x"))
        .andExpect(status().isForbidden());
    api.doPost(TL, one + "/insurers/" + lead + "/reserve" + c(), Map.of("amount", 1000, "reason", "Estimate"))
        .andExpect(status().isOk());
    api.doPost(OFFICER, one + "/insurers/" + lead + "/adjuster" + c(), Map.of("adjusterCode", "CRAWFORD"))
        .andExpect(status().isForbidden());
    api.doPut(OFFICER, one + "/insurers/" + lead + "/share" + c(), Map.of("sharePct", 70)).andExpect(status().isOk());
    api.doPost(OFFICER, one + "/claimant" + c(), Map.of("claimantName", "X", "reason", "OTHERS"))
        .andExpect(status().isForbidden());
    api.doPost(TL, one + "/claimant" + c(), Map.of("claimantName", "Third Party", "reason", "OTHERS"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.flags.claimantOverridden").value(true));
    api.doPost(OFFICER, one + "/reported-date" + c(), Map.of("reportedDate", LocalDate.now().toString(), "reason", "DATA_CORRECTION"))
        .andExpect(status().isOk());
    api.doPost(OFFICER, one + "/premium-check" + c(), Map.of())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value("UNPAID"));
    api.doPost(OFFICER, one + "/authorize" + c(), Map.of()).andExpect(status().isUnprocessableEntity());
    api.doPost(OFFICER, one + "/refresh-cover" + c(), Map.of()).andExpect(status().isOk());
    api.doPost(OFFICER, one + "/latest-version" + c(), Map.of()).andExpect(status().isUnprocessableEntity());
    api.doPost(OFFICER, one + "/updates" + c(), Map.of("updateDate", LocalDate.now().toString(), "source", "EMAIL", "remarks", "Acknowledged"))
        .andExpect(status().isCreated());
    api.doPost(OFFICER, one + "/loss-advice" + c(), Map.of("recipients", List.of(Map.of("insurerCode", LEAD, "to", List.of("claims@insurer.test")))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].insurerCode").value(LEAD));
    api.doPost(OFFICER, BASE + "/location-refs", Map.of("companyId", fx.company(), "arn", arn, "itemNo", 2, "insurerCode", LEAD, "reference", "API-LOC"))
        .andExpect(status().isCreated());
    ...
    api.doPut(OFFICER, one + "/locations/2" + c(), Map.of("description", "Garage")).andExpect(status().isOk());
    api.doDelete(OFFICER, one + "/locations/2" + c()).andExpect(status().isOk());
    api.doPost(OFFICER, one + "/insurers" + c(), Map.of("insurerCode", " ")).andExpect(status().isUnprocessableEntity());
    api.doPut(OFFICER, one + "/loss" + c(), Map.of("lossDate", LocalDate.now().minusDays(2).toString(), "lossNature", "FIRE", "claimType", "PROPERTY", "lossDescription", "Kitchen and dining"))
        .andExpect(status().isOk());
  }
