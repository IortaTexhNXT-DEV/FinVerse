...
import com.iortatechnxt.brokerverse.collections.demo.CollectionsDemoData;
...
/**
 * Loads the demo profile in its own database and checks the Collections storyline: the worklist
 * refreshed from the demo bookings, dispositions of every kind handed to Cashiering and Commission
 * through the outbox, a temporary reassignment and the files published with their availability,
 * each done by the demo user whose job it is; a second start changes nothing.
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles({"test", "demo"})
@AutoConfigureEmbeddedDatabase(provider = DatabaseProvider.ZONKY)
class CollectionsDemoDataIT {

  @Autowired private JdbcTemplate jdbc;
  @Autowired private CollectionsDemoData demo;
  ...
  @Test
  void theWorklistIsRefreshedFromTheDemoBookingsAndWorked() {
