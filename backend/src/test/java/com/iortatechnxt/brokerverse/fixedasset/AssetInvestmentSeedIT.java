...
import com.iortatechnxt.brokerverse.fixedasset.seed.FixedAssetSeedData;
...
import com.iortatechnxt.brokerverse.investment.seed.InvestmentSeedData;
...
/**
 * Runs both seed data runners (normally active only with the seed profile) inside a rolled back
 * transaction, checks the data set and exercises every fixed asset and investment report on it.
 */
@IntegrationTest
@Transactional
class AssetInvestmentSeedIT {
  ...
  @Test
  void seedDataIsBuiltIdempotentlyAndEveryReportRunsAndExports() {
    FixedAssetSeedData assetSeed =
        new FixedAssetSeedData(
            companies, branches, categories, assets, assetService, lifecycle, depreciation);
    InvestmentSeedData investmentSeed =
        new InvestmentSeedData(
            companies,
            branches,
            portfolios,
            holdings,
