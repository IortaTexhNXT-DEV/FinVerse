...
import com.iortatechnxt.brokerverse.journal.seed.OpeningBalanceSeedData;
...
/**
 * Loads the seed profile in its own database (the context and database are shared with the other
 * seed data tests) and checks the bank opening balances: one OPENING journal per branch on 1
 * January 2026, posted once; every bank account of the seed company in credit at every month end of
 * 2026 after all seed activity, for the company and for each branch; and the seed bank
...
class OpeningBalanceSeedDataIT {
  ...
  @Autowired private OpeningBalanceSeedData loader;
  ...
  private int openingJournals() {
    return jdbc.queryForObject(
        ...
        Integer.class,
        company(),
        OpeningBalanceSeedData.SOURCE,
        OpeningBalanceSeedData.REFERENCE + "-%",
        Date.valueOf(OpeningBalanceSeedData.TAKE_ON));
  }
  ...
  void openingJournalsArePostedOnceAndKeepEveryBankAccountInCredit() {
    ...
    loader.run(null);
    assertThat(openingJournals()).isEqualTo(3);
    assertThat(balance(BDO, OpeningBalanceSeedData.TAKE_ON)).isEqualByComparingTo("150000000.00");
    assertThat(balance(BPI, OpeningBalanceSeedData.TAKE_ON)).isEqualByComparingTo("20000000.00");
  ...
  void bankStatementBringsTheOpeningBalanceForwardAndStillReconciles() {
    ...
    assertThat(first.getValueDate()).isEqualTo(OpeningBalanceSeedData.TAKE_ON);
