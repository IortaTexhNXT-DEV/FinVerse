...
import com.iortatechnxt.finverse.reserves.demo.ReservesDemoData;
...
import com.iortatechnxt.finverse.reserves.service.ReserveSummary;
import com.iortatechnxt.finverse.reserves.service.TakafulSettingService;
...
/**
 * Runs the reserves demo data loader (normally only with the demo profile) inside a rolled back
 * transaction, checks the runs it creates and its idempotency, and runs and exports every actuarial
 * report on that data.
 */
@IntegrationTest
@Transactional
class ReservesDemoAndReportsIT {
  ...
  private ReservesDemoData loader() {
    return new ReservesDemoData(organization, products, parameters, takaful, runs, users);
  }
  ...
  @Test
  void demoRunsAreBuiltIdempotentlyAndEveryReportRunsAndExports() {
