...
| **Renewal (BRD being analysed)** | `ClaimExperienceQueryService` (public read API of `brokerclaims`): per ARN / policy year: claim count, open count, statuses, paid, O/S, total; renewal depends on `brokerclaims`, never the reverse | 030/040; RN p.50, 86 |
...
| Renewal (BRD being analysed) | Consumes `brokerclaims.service.ClaimExperienceQueryService.summary(arn, policyYear)` for "With Claim (Y/N)", number and status of claims (RN p.50, p.86); the "Total Loss Claim" non-renewal reason (RN p.82) needs a total-loss indicator that BRD-7 does not define (CLQ28) | Ask to the Renewal analysis |
| Customer Servicing Facility (BRD being analysed) | Claim documents (BRCSF-009) reachable through the client records port and attachments of entity type `BrokerClaim` | Note to the CSF analysis |
...
| BDO SSO / Active Directory (Windows credentials) | NFR 1.01 | Existing login | Q42 |
