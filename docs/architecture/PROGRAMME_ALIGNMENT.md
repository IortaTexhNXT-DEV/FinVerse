...
| CI / CD | HW sheet: GitLab Runner, AWS SSM, kubectl / helm on a bastion; infra diagram: GitHub + Jenkins; diagram: ArgoCD GitOps + Helm, Terraform + Crossplane | GitHub Actions (ci.yml: mvn verify, SonarQube, npm verify); plain Kubernetes manifests; no Helm chart | Gap | Helm chart (or Kustomize overlays) per environment; pipeline on the BDO toolchain once named | IQ29 |
...
|---|---|---|---|---|---|---|
| DEV | brokerverse-backend | 1 | 2 | 500m / 1.5 Gi | 2 / 3 Gi | Demo data; jobs run on the single pod |
...
| IQ29 | CI / CD toolchain | (Priority 2) Which toolchain applies: GitLab (HW sheet), GitHub + Jenkins (infra diagram) or ArgoCD + Helm (diagram)? Container registry, image base (RHEL UBI or Alpine), SonarQube instance. | IER sheets |
