...
| `BROKERVERSE_JOB_PASSWORD_EXPIRY_NOTICE_CRON` | no | `0 0 22 * * *` | Spring cron (UTC) of `PASSWORD_EXPIRY_NOTICE` (security, UAM-NFR-36; 06:00 PHT, `AUTH_MODE` LOCAL only: notifies the users whose password expires within 7 days of `PASSWORD_MAX_AGE_DAYS`). Property `brokerverse.jobs.password-expiry-notice-cron`. |
