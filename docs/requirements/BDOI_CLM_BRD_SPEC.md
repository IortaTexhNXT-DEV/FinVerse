...
[`docs/modules/BROKER_CLAIMS.md`](../modules/BROKER_CLAIMS.md).

| Status | Meaning | Rows |
|---|---|---|
| Built | Works as the acceptance criteria say | 22 |
| Built with parked item | Works; one element waits for BDOI (question in brackets) | 21 |
| Not built | - | 0 |
| **Total** | | **43** |

The parked elements are the BDOI values and definitions (CLQ01, CLQ04-CLQ08, CLQ11, CLQ15, CLQ16,
CLQ20), the location lists per endorsement (CLQ02), the insurer channels (CLQ17), the report shared
drive (CLQ21), the legacy claims migration (CLQ14) and the dynamic report builder (Q40). Claim
proceeds through BDOI (CLQ10) stay out of scope: Claims Handling posts no accounting entry.
End-to-end evidence: `api/ClaimsEndToEndApiIT` (booking, claim blocked on the unpaid cover,
cashiering payment, authorization code, "With BDOI - For Premium Remittance", special remittance
confirmed through the claims feed, remittance batch and DV, FULLY_REMITTED notice, statuses through
the matrix, settlement on the LOA, reopen and the reports), `api/ApiSmokeIT` (every Claims list
read) and `security/PersonaMenusIT` with `frontend/src/navigation/personaMenus.test.ts` (menus of
the Claims and Marketing roles).
