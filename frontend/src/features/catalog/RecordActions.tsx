 ...
 */
export function RecordActions({ kind, record, refresh }: Readonly<Props>) {
  ...
  const canAuthorize = can('MASTER_AUTHORIZE') && awaitsOtherChecker(record, user?.username);
  const canDeactivate = can('MASTER_MAINTAIN') && record.recordStatus === 'ACTIVE';
