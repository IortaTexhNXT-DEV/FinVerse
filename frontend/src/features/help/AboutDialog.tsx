...
/** About iNXT FinVerse: product, version, build and vendor. */
export function AboutDialog({ open, onClose }: Readonly<{ open: boolean; onClose: () => void }>) {
  ...
  const a = about.data;
  return (
    <Modal title="About iNXT FinVerse" open={open} onClose={onClose}>
      <div className="stack" style={{ textAlign: 'center' }}>
        <div className="brand-mark" style={{ margin: '0 auto' }} aria-hidden="true">
          FV
        </div>
        <h2 style={{ margin: 0 }}>{a?.product ?? 'iNXT FinVerse'}</h2>
