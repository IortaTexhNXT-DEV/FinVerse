...
import type { Amounts, RemittanceType } from './api';
import { TYPE_LABELS } from './remittanceLabels';
import './remittance.css';

/** Totals strip of a batch (RMTID.002 addendum: amounts are read-only). */
export function TotalsStrip({ totals, currency }: Readonly<{ totals: Amounts; currency: string }>) {
  const items: { label: string; value: number; strong?: boolean }[] = [
    ...
    { label: `Payable (${currency})`, value: totals.payable, strong: true },
  ];
  return (
