...
3. **Capture.** Run this from the repository root:
   ```
   BASE=http://localhost:5173 API=http://localhost:8080 \
...
The script waits until `API/actuator/health/readiness` reports UP. The demo start-up runners (booking, ledger replay, Operations demo data) run after "Started" is logged, so capturing earlier would show empty Operations screens.
